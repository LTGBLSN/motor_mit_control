...
2025.11.7____基本驱动历程完工，电机数据整体打包成单个结构体，固件原因回传速度值毛刺较多，电机转速建议根据位置值微分取得。已经完成can1can2同时驱动两个电机，此固件id仅可从1到4，因此大于4个电机时请分为两路can驱动。温度值暂未做测试，但接收函数已写好
...
固件存储在仓库主文件夹→小米电机→购买→mi.bin

使用stlinkUtility下载或其他下载方式
