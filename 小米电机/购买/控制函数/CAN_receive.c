...
//		vTaskDelay(0.1);
	...
	int error=HAL_CAN_AddTxMessage(hcan, &Tx_Header, pData, (uint32_t*)CAN_TX_MAILBOX1);
...
int DM8009_enable_motor(uint16_t id)
{
	static uint8_t date_enable[8] = {0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xfc};
  return CANx_SendStdData(&CHASSIS_CAN,id,date_enable,8);	
}
...
void leg_goto_zero_position()
{
	  DM8009_ctrl_motor(&CHASSIS_CAN, 1, 0, 0, 0, 2, +1.5);
		vTaskDelay(2);
		DM8009_ctrl_motor(&CHASSIS_CAN, 2, 0, 0, 0, 2, -1.5);
		vTaskDelay(2);
		DM8009_ctrl_motor(&CHASSIS_CAN, 3, 0, 0, 0, 2, -1.5);
		vTaskDelay(2);
		DM8009_ctrl_motor(&CHASSIS_CAN, 4, 0, 0, 0, 2, +1.5);
		vTaskDelay(2000);
}
void joint_motor_enable()
{
    int init_error;
    init_error = DM8009_enable_motor(1);
    vTaskDelay(500);
    init_error = DM8009_enable_motor(2);
    vTaskDelay(500);
    init_error = DM8009_enable_motor(3);
    vTaskDelay(500);
    init_error = DM8009_enable_motor(4);
    vTaskDelay(500);
}